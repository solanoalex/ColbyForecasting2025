...
```{r}
shrimp_na = read_observations(individual_na = F)
shrimp = read_observations()

shrimp |> count(basisOfRecord)
...
```{r}
ggplot(data = shrimp_na,
       mapping = aes(x = factor(month, month.abb))) + 
  geom_bar() + 
  labs(x = "month", title = "Counts per Month")
...
plot(mask, breaks = "equal", axes = TRUE, reset = FALSE)
plot(st_geometry(shrimp_na), pch = ".", add = TRUE)

plot(mask, breaks = "equal", axes = TRUE, reset = FALSE)
plot(st_geometry(shrimp), pch = ".", add = TRUE)
```

```{r}
coast = read_coastline()
ggplot(data = shrimp_na) +       # create a plot object
...
coast = read_coastline()
ggplot(data = shrimp) +       # create a plot object
  geom_sf(alpha = 0.1,      # add the points with some tweaking of appearance
          size = 0.7) +  
  geom_sf(data = coast,     # add the coastline
          col = "orange")
...
```{r}
coast = read_coastline()
ggplot(data = shrimp_na) +                  # create the plot
  geom_sf(alpha = 0.1, size = 0.7) +   # add the points
  geom_sf(data = coast,                # add the coast
          col = "orange") +
  facet_wrap(~factor(month, month.abb))# make a montage by month
