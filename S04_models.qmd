...
```{r greedy approach}
#for the greedy selection method
for(month in month.abb){
  
  #first, we need to create the model input, the training data and test data
  model_input = read_model_input(scientificname = "Pandalus borealis", 
                               approach = "greedy", 
                               mon = month)
  if(!is.null(model_input)){
    cfg = read_configuration(version = paste("g_", month, sep = ""))
    db = brickman_database()
    covars = read_brickman(db |> filter(scenario == "PRESENT", interval == "mon"))|>
      select(all_of(cfg$keep_vars))
    ...
    # next, we will start creating the workflow
    wflow = workflow()
    ...
    # next, we need to make the model for the month
    model = rand_forest() |>
    set_mode("classification") |>
    set_engine("ranger", probability = TRUE, importance = "permutation")
    ...
    # lastly, we need to save the workflow
    write_workflow(fitted_wflow, version = cfg$version)
  }
}
```
...
```{r conservative approach}
#for the conservative selection method
for(month in month.abb){
  ...
  model_input = read_model_input(scientificname = "Pandalus borealis", 
                               ...
                               mon = month)
  if(!is.null(model_input)){
    cfg = read_configuration(version = paste("c_", month, sep = ""))
    db = brickman_database()
    covars = read_brickman(db |> filter(scenario == "PRESENT", interval == "mon"))|>
      select(all_of(cfg$keep_vars))
