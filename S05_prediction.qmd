...
# write_prediction(nowcast, file = file.path(path,"g_Aug_RCP45_2020.tif")) # the nowcast is the same
write_prediction(forecast_2055, file = file.path(path, "g_Aug_RCP45_2055.tif"))
write_prediction(forecast_2075, file = file.path(path, "g_Aug_RCP45_2075.tif"))
```
...
plot(nowcast['.pred_presence'], main = "Nowcast August", 
     axes = TRUE, breaks = seq(0, 1, by = 0.1), reset = FALSE)
plot(coast, col = "orange", lwd = 2, add = TRUE)
plot(st_geometry(shrimp), pch = ".", col="blue",add = TRUE)
```
