...
```{r rcp45 greedy approach}
for(month in month.abb){
  
  cfg = try(read_configuration(version = paste("g_", month, sep = "")))
  
  if(inherits(cfg, "try-error")){
    message("configuration not found: ", paste("g_", month, sep = ""))
  }
  else{
    db = brickman_database()
    covars = read_brickman(db |> filter(scenario == "PRESENT", interval == "mon")) |>
    ...
    write_prediction(nowcast, file = file.path(path, paste("g_", month, "_RCP45_2020.tif", sep = "")))
    write_prediction(forecast_2055, file = file.path(path, paste("g_", month, "_RCP45_2055.tif", sep = "")))
    write_prediction(forecast_2075, file = file.path(path, paste("g_", month, "_RCP45_2075.tif", sep = "")))
    ...
    plot(rcp45['.pred_presence'], 
       hook = plot_coast,
       axes = TRUE, breaks = seq(0, 1, by = 0.1), join_zlim  = TRUE, reset = FALSE, main = paste("RCP45 for ", month, sep=""))3
...
```{r rcp45 side by side comparisons} 
coast = read_coastline()
...
```{r rcp45 nowcast with presence points}
coast = read_coastline()
plot(nowcast['.pred_presence'], main = "Nowcast August", 
     axes = TRUE, breaks = seq(0, 1, by = 0.1), reset = FALSE)
plot(coast, col = "orange", lwd = 2, add = TRUE)
plot(st_geometry(read_observations(individual_na = F)), pch = ".", col="blue",add = TRUE)
```



```{r rcp45 conservative approach}
for(month in month.abb){
  
  cfg = try(read_configuration(version = paste("c_", month, sep = "")))
  
  if(inherits(cfg, "try-error")){
    message("configuration not found: ", paste("c_", month, sep = ""))
  }
  else{
    db = brickman_database()
    covars = read_brickman(db |> filter(scenario == "PRESENT", interval == "mon")) |>
    ...
    covars_rcp45_2075 = read_brickman(db |> filter(scenario == "RCP85", year == 2075, interval == "mon")) |>
      select(all_of(cfg$keep_vars)) |>
      slice("month", month) 
    
    forecast_2075 = predict_stars(wflow, covars_rcp45_2075)
    
    covars_rcp45_2055 = read_brickman(db |> filter(scenario == "RCP85", year == 2055, interval == "mon")) |>
      select(all_of(cfg$keep_vars)) |>
      slice("month", month) 
    forecast_2055 = predict_stars(wflow, covars_rcp45_2055)
    ...
    write_prediction(nowcast, file = file.path(path, paste("c_", month, "_RCP85_2020.tif", sep = "")))
    write_prediction(forecast_2055, file = file.path(path, paste("c_", month, "_RCP85_2055.tif", sep = "")))
    write_prediction(forecast_2075, file = file.path(path, paste("c_", month, "_RCP85_2075.tif", sep = "")))
    plot_coast = function(){
    plot(coast, col = "orange", lwd = 2, add = TRUE)
    }
    ...
    plot(rcp45['.pred_presence'], 
       hook = plot_coast,
       axes = TRUE, breaks = seq(0, 1, by = 0.1), join_zlim  = TRUE, reset = FALSE, main = paste("RCP85 for ", month, sep=""))
  }
}
